...
private constructor(private val serviceConnection: PodcastServiceConnection) :
    ...
    var playlist: ArrayList<Articles> = ArrayList()
        ...
        if (playlist.isEmpty()) {
            ...
            jcPlayerService?.let { service ->
                if (service.getMediaPlayer() != null) {
                    ...
                    if (currentPosition + seekForwardTime <= service.getMediaPlayer()!!
                            .getDuration()
                    ) {
    ...
    private fun getNextAudio(): Articles? {
    ...
    private fun getPreviousAudio(): Articles {
        return if (onShuffleMode) {
            playlist[Random().nextInt(playlist.size)]
        } else {
            try {
